...
def get_game_weather(stadium_name: str, game_date: str) -> Dict:
    """
    ...
    if stadium_name not in STADIUM_COORDINATES:
        ...
        for stadium, coords in STADIUM_COORDINATES.items():
            ...
            print(f"⚠️ Stadium '{stadium_name}' not found in coordinates, assuming outdoor")
    ...
    try:
        ...
        print(f"⚠️ Failed to fetch weather for {stadium_name} on {game_date}: {e}")
...
def adjust_for_weather(prob_over: float, prop_type: str, weather: Dict) -> float:
    ...
    if prop_type in ['passing_yards', 'passing_tds'] and weather['temp_f'] < 32:
        ...
        print(f"❄️ Cold penalty: -{cold_penalty:.1%} for {prop_type} (temp: {weather['temp_f']:.1f}°F)")
    ...
    if weather['precipitation_mm'] > 5.0:
        ...
        if prop_type in ['passing_yards', 'passing_tds']:
            ...
            print(f"🌧️ Rain penalty: -{rain_penalty:.1%} for {prop_type} (precip: {weather['precipitation_mm']:.1f}mm)")
            ...
            print(f"🌧️ Rain bonus: +{rain_penalty * 0.5:.1%} for {prop_type} (precip: {weather['precipitation_mm']:.1f}mm)")
...
if __name__ == "__main__":
    ...
    print("Original prob: {:.3f}, Adjusted: {:.3f}".format(test_prob, adjusted))</content>
<parameter name="filePath">c:\Users\gmalb\Downloads\nfl-predictions\player_props\weather.py