        ...
        if os.path.exists(log_path):
            ...
            if len(betting_log) > 0:
                ...
                if len(completed_bets) > 0:
                    ...
                    confidence_performance = completed_bets.groupby('confidence_tier', observed=True).agg({
                    ...
                    weekly_data = completed_bets.groupby('week', observed=True).agg({
                        'bet_result': lambda x: (x == 'win').mean() * 100,
                        'bet_profit': 'sum'
                    }).round(2)
                    ...
                    bet_type_performance = completed_bets.groupby('bet_type', observed=True).agg({
                        'bet_result': lambda x: (x == 'win').mean() * 100,
                        'bet_profit': 'sum'
                    }).round(2)
...
# =============================================================================
# Navigation - icons declared here, not in filenames (macOS/Docker compatible)
# Only runs when executed by Streamlit (not when imported as a module in tests)
# =============================================================================
try:
...
if _has_runtime:
    # Detect Streamlit Community Cloud (env var set automatically by the platform)
    _is_cloud = bool(os.environ.get("IS_STREAMLIT_CLOUD") or os.environ.get("STREAMLIT_SHARING_MODE"))
    ...
    # ── Sidebar enhancements applied on every page ─────────────────────────
    # 1. Hide the top-right settings / hamburger menu on Streamlit Cloud only.
    if _is_cloud:
        st.markdown(
            """
            <style>
    ...
    # 2. Brand logo below the navigation (visible everywhere).
    _logo_path = os.path.join(DATA_DIR, "gridiron-oracle-transparent.png")
